...
class AudioGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, filename_temp):
        ...
        for i, ID in enumerate(filename_temp):
            ...
            # for j in range(2):
            #     y[i, :, :, :, j] = np.load(self.database_path+'AV_model_database/single/' + info[j + 1])
...
class AVGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, filename_temp):
        ...
        # return [tf.convert_to_tensor(X1, dtype=tf.float32), tf.convert_to_tensor(X2, dtype=tf.float32)], tf.convert_to_tensor(y, dtype=tf.float32)
        # return [X1, X2], y
        return [tf.constant(X1, dtype=tf.float32), tf.constant(X2, dtype=tf.float32)], tf.constant(y, dtype=tf.float32)
